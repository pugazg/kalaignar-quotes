...
- [`keyword-index-design.md`](keyword-index-design.md) — keyword-index construction protocol, evidence rules, working-vocabulary lifecycle, 25-quote batching, validation and final consolidation plan. Design version **0.1**; sequential keyword assignment and the full consistency pass are complete for `KQ-CCM-0001`–`KQ-CCM-0497` (**497 / 497**), with vocabulary freeze and consolidated publication next.
- [`keyword-vocabulary.md`](keyword-vocabulary.md) — consistency-reviewed working keyword vocabulary version **0.9** with **182 stable keys**; the pass added `patience` and migrated the one-off `hindikkara-state` key into `hindi-state`, producing no net key-count change. The vocabulary is not yet frozen.
- [`keyword-consistency-review.md`](keyword-consistency-review.md) — completed full consistency pass across all **497 / 497** keyword assignments. It records **21 quote-row corrections across 12 batch files**, the version 0.9 vocabulary decisions, preservation of source verification states and the publication gate that follows.
...
- [`keyword-0476-0497.md`](keyword-0476-0497.md) — twentieth and final keyword-index construction batch for `KQ-CCM-0476`–`KQ-CCM-0497`; overall sequential keyword coverage is **497 / 497 quotes**.
...
Sequential keyword-index construction and the required full consistency pass are complete under design version **0.1**. All twenty batches cover **497 / 497 quotes**, and the pass confirmed a contiguous set of **497 unique rows**, one to five valid keywords per quote, and preservation of the canonical verification states: **496 `verified_from_scan` and 1 `needs_review` (`KQ-CCM-0391`)**. The pass applied **21 quote-row corrections across 12 batch files** to backfill later-established precise keys, remove broader fallbacks where appropriate, and align repeated concepts. [`keyword-vocabulary.md`](keyword-vocabulary.md) is now consistency-reviewed working version **0.9** with **182 keys**: it adds `patience` and migrates the one-off `hindikkara-state` key into `hindi-state`, leaving the total key count unchanged. `KQ-CCM-0391` remains `needs_review`, its obscured terminal glyph remains unresolved, and no canonical quote file was modified. The next planned derived-discovery activity is to **freeze the vocabulary as version 1.0 and publish the consolidated `keyword.md` index**; details of the completed pass are recorded in [`keyword-consistency-review.md`](keyword-consistency-review.md).
